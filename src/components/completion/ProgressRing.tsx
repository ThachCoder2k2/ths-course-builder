 ...
 *
 * Thiết kế ghi "Trọng số hoàn thành 100%": đây là phần NỘI DUNG đã đi qua (tính theo độ
 * dài video), không phải mức nắm. Hai thứ đó khác nhau và màn này nói cả hai — vòng này
 * là hoàn thành, còn radar và sơ đồ mới là mức nắm.
 */
const CO = 240;
const DAY = 20;
...
export function ProgressRing({ giaTri, nhan }: { giaTri: number; nhan: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-lg">
      <svg viewBox={`0 0 ${CO} ${CO}`} className="h-auto w-[200px] max-w-full" role="img" aria-label={`${nhan}: ${phanTram}%`}>
        <circle cx={CO / 2} cy={CO / 2} r={BAN_KINH} fill="none" stroke="#E9EAEB" strokeWidth={DAY} />
        {/* Quay -90° để vạch bắt đầu từ đỉnh, đúng như thiết kế. `pathLength` cố định 1
            nên phần vẽ tính bằng tỉ lệ, khỏi phải nhân với chu vi ở hai chỗ. */}
        <circle
          cx={CO / 2}
          cy={CO / 2}
          r={BAN_KINH}
          ...
          className="cp-ring"
        />
        <text x={CO / 2} y={CO / 2 - 6} textAnchor="middle" className="fill-[#535862] text-[13px]">
          {nhan}
        </text>
        <text x={CO / 2} y={CO / 2 + 30} textAnchor="middle" className="fill-[#181D27] text-[34px] font-bold">
          {phanTram}%
        </text>
      </svg>
    </div>
