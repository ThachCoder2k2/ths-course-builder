...
export default function CourseCard({ course }: { course: Course }) {
  ...
  return (
    <Link to={'/courses/' + course.slug} className="group block h-full">
      <article className="flex h-full flex-col overflow-hidden rounded-card border border-line bg-surface shadow-card transition-shadow hover:shadow-pop">
        ...
        <div className="flex flex-1 flex-col gap-3 p-4">
          <h3 className="line-clamp-2 font-bold text-ink-900 transition-colors group-hover:text-brand-700">
            {course.title}
          </h3>
          ...
          {instructor ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-ink-500">{instructor.name}</span>
            </div>
          ) : null}
