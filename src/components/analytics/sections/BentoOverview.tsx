...
function JumpTile({ title, onClick, children }: { title: string; onClick: () => void; children: ReactNode }) {
  return (
    <Reveal className="h-full">
      <button
        type="button"
        onClick={onClick}
        className="group flex h-full w-full flex-col gap-lg rounded-card border border-secondary bg-primary p-xl text-left shadow-card transition hover:-translate-y-0.5 hover:border-brand-alt hover:shadow-pop"
      >
...
export function BentoOverview({ a, onJump }: { a: LearnerAnalytics; onJump: (id: string) => void }) {
  ...
  const k = a.kpis;
  const skills = [...a.mastery.skills];
  const strong = skills.slice().sort((x, y) => y.current - x.current)[0];
  const weak = skills.slice().sort((x, y) => x.current - y.current)[0];
  ...
  return (
    <section id="tong-quan" className="flex scroll-mt-28 flex-col gap-xl">
      <Reveal>
        <div className="flex flex-col gap-2xl rounded-card border border-brand-200 bg-[linear-gradient(120deg,#F0F6FE_0%,#F4F3FF_100%)] p-3xl lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-xl">
            ...
            <div className="flex flex-col gap-md">
              <div className="flex flex-col gap-xxs">
                ...
                <p className="text-sm text-tertiary">Đang học “{a.learner.currentCourse}”</p>
              </div>
          ...
          <div className="flex shrink-0 flex-col items-center gap-md">
            <span className="text-sm font-medium text-tertiary">Sức khỏe học tập</span>
      ...
      <div className="grid grid-cols-1 gap-xl md:grid-cols-2 xl:grid-cols-4">
        <JumpTile title="Thành thạo" onClick={() => onJump('thanh-thao')}>
          <div className="flex flex-col gap-md">
            <MiniBar label={`Mạnh nhất · ${strong.skill}`} value={strong.current} color={STATUS.good} />
            <MiniBar label={`Cần củng cố · ${weak.skill}`} value={weak.current} color={STATUS.warning} />
          </div>
        </JumpTile>

        <JumpTile title="Nên học tiếp" onClick={() => onJump('goi-y')}>
          <div className="flex items-start gap-md">
            ...
            <div className="flex min-w-0 flex-col">
              ...
              <span className="text-xs text-brand-secondary">Phù hợp {top.fit}%</span>
            </div>
          </div>
        </JumpTile>

        <JumpTile title="Điểm đánh giá" onClick={() => onJump('danh-gia')}>
          <div className="flex items-center gap-lg">
            <RadialRing value={quizAvg} size={64} stroke={8} centerLabel={`${quizAvg}`} />
            <span className="text-sm text-tertiary">Điểm quiz trung bình trên {a.assessment.quizzes.length} bài</span>
          </div>
        </JumpTile>

        <JumpTile title="Nhịp học" onClick={() => onJump('nhip-hoc')}>
      ...
      <AiInsightCard label="Tổng quan">
        Tuần này bạn học đều hơn tuần trước và giữ được chuỗi {k.streak.value} ngày. Bạn mạnh phần {strong.skill}, còn {weak.skill} thì nên
        dành thêm thời gian. Cuộn xuống hoặc bấm vào từng thẻ ở trên để xem kỹ, hoặc để mình dẫn bạn đi một vòng nhé.
      </AiInsightCard>
    </section>
  );
}
